...
namespace GDS.Demos.Basic {
    ...
    public class CraftingBench : ListBag {
        ...
        public CraftingBench() {
            Size = 3;
            OnCollectionChanged += UpdateOutcome;
        }
        ...
        public override Result MoveItem(Item item, Bag targetBag) {
            var shouldCraft = OutcomeSlot.Value.Item == item;
            var result = base.MoveItem(item, targetBag);
            if (shouldCraft && result is PlaceItemSuccess r) { return result.MapTo(new CraftItemSuccess(r.Item)); }
            return result;
        }
    }
}
