    ...
    public class EventBus {
        ...
        public void Publish(CustomEvent Event) {
            ...
            foreach (var entry in byAnyType) {
                ...
                foreach (var sub in entry.Value) sub.Invoke(Event);
            }
        }

        void PrettyPrint(CustomEvent Event) {
            LogResultEvent(Event);
        }

        void LogResultEvent(object e) {
            var str = e switch {
                Success => $"[Success::{e.GetType().Name.Green()}] {e}",
                Fail => $"[Fail::{e.GetType().Name.Red()}] {e}",
                _ => $"[Event::{e.GetType().Name.Gray()}] {e}"
            };
            Debug.Log(str);
        }
    }
